...
| 4 | **同事開自己的文件可能踩到「註腳裡有 frame」→ 引擎死** | **操作** | **修不了**（上游 [038](../findings/038-a-frame-inside-a-footnote-wedges-the-engine-on-selection.md)／[040](../findings/040-idleslockguard-waits-on-a-condition-an-emscripten-build-can-never-set.md)）。可行的「完成」是三選一：<br>(a) 產品側先擋掉那個構造；<br>(b) 讓逾時復原夠好看（#33 已做一半）；<br>(c) demo 只用準備好的文件，並把限制講明 | 取決於選哪個；(c) 幾乎零成本 |

**所以真正的路徑是**：#49 → #50 → 決定第 4 項走 (a)/(b)/(c) → 然後 #48／E2-B。

...
## #48：SPEC-E2-B 已寫到 v3，第 3 節閘門已跑完

**流程**：規格 v1 → codex 對抗性審查打掉十一項 → v2 → 預測先 commit → 寫 harness → 跑 → v3。

**審查打掉的兩個致命項**（我都自己核對過）：
1. **G3 的判準接錯機制**——v1 要求跨段「以 typed `multiBlock` 拒絕」，
   但 `multiBlock` 算的是 **barrier 自己選的那一段**，而 barrier 用 `.uno:SelectText`
   永遠只選一段、判讀又在派送之後。**跨段輸入結構上走不到那個拒絕**，
   那一臂等於在等一個不會發生的事。
2. **正向臂證明不了範圍存在**——五個動作都是段落層級的，
...
| **G3**（跨段） | **失敗 3/3**，如預測——**但文件結果是對的**（兩段都變、其餘未動）。失敗的是「回報成功」而 barrier 只驗得了一段 |
| **G1**（同段多矩形） | **void**：`第三段跨行 gamma` 其實是段落很高、不是跨行。**這一格仍然是暗的**，現有 fixture 沒有會換行的段落 |

**依 3.7 落在「只有 G3 失敗」分支**：範圍派送可以進 ABI，但跨段要嘛派送前 typed 拒絕、
要嘛改成驗證範圍內每一段——**兩條都還沒做，凍結仍被擋著**（第 5 節的協定問題也全未解）。

> **三個比較器錯誤全是我的，全靠資料荒謬才抓到**，其中一個要特別記：
> **事後寫的分析器把預先登記的判準悄悄換掉了**（給 G3 `allowed=2` 判成通過，
> 而 `PREDICTION.md` 在 harness 存在之前就寫著那是失敗），**而且偏離的方向讓那一臂變綠**。

> **閘門 harness 沒有 Makefile target**，是手動 `cp` 進 `dist/` 的——
> finding 042：改 Makefile 會重連結，等於在閘門跑之前把受測 artifact 換掉。
> **和 `f049_native_...cpp` 的語法檢查一起，留到下次刻意 relink 時補。**

## 還開著的

| # | 事情 | 卡在誰 |
|---|---|---|
| **48** | **規格 v3 已寫、閘門已跑**（見上）。**下一步有三條，都可做**：(a) 跨段的處置——派送前 typed 拒絕或改成驗證每一段，各要自己的預測與一輪；(b) **做一份會在頁寬換行的 fixture** 把 G1 那一格補起來；(c) 第 5 節的協定問題（worker 的 `endsWith("V1")` 閘門、action map、產品 client 要求 `changed === true`、產品 worker 不轉發 `formatBarrier`） | 可做 |

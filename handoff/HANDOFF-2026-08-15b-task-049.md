...
| # | 事情 | 狀態 |
|---|---|---|
| 1 | **拖曳選一段再按按鈕會失敗** | **已修並已驗**（任務 #49）。根因是上游，我方加了標註過的 workaround；A3／A4／A5 重掃全過，**SPEC E2-A 的縮限 4 已撤**（v23） |
...
| # | 事情 | 卡在誰 |
|---|---|---|
| **48** | **寫 SPEC-E2-B** | **不再被擋，而且是 demo 出場條件的最後一項**。E2-A v23 說「**方可**」凍結 B 的 ABI——是允許不是應當。第一個要補的量測是**縮限 7**：在範圍選取上派送格式動作 |
...
| **44** | 判定綁定的 `summary.json` 與規格矛盾 | 見 finding 044。要覆寫判定綁定檔案，刻意為之 |

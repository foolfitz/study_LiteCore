...
六項全做完，commit `273c317` → `50c4eac`。交接寫在
[`HANDOFF-2026-08-16c-queue-shrunk.md`](HANDOFF-2026-08-16c-queue-shrunk.md)。

| 任務 | 結果 |
|---|---|
| T1 | **佇列項撤銷**——數字本來就拿得到，而且佇列指名的那個數字是常數 |
| T2 | **3b 的不一致解掉了**：瀏覽器那個 0 從來不是讀回；掉出一個同族新項 |
| T3 | `check_relink_queue.py` ＋ 佇列 JSON，fail closed、雙向、十個突變 |
| T4 | 凍結時機的斷言進了 D0 的 runner 與判定器，七條自我測試 |
| T5 | 觀測旗標與三個投影欄位進樹（只編 object）；重排刻意不做，記成決定 |
| T6 | 那段等待**在引擎那一側**（99%），但不得再往下猜 |

**排序的賭注贏了**：T1 排第一是因為它可能讓後面少做一件事——它確實拿掉了一項，
而且發現那一項瞄準的是一個不會動的數字。

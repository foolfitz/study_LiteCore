...
| shell | **v27 `55a91f837f210e8e…`** — `web/e2-editor-app.js` twice in one day (findings 066 then 067). It is the bundle's entrypoint, so every product fix mints a generation. Bundle re-minted, matrix binding re-stated, D0 re-run: `d0-shell-v26/` and `d0-shell-v27/`, both `pass: true` on both browsers |
...
| **067 — the PRODUCT half is fixed (shell v27)** | Enter is now bound in the page's own keydown handler — paragraph break, or line break with Shift — so it never reaches the frozen adapter. `the-enter-key-reaches-the-document` passes on Chrome (Enter 10 → 11 paragraphs, markers no longer together; Shift+Enter 12 → 12 with a `<text:line-break/>`), abstains on Firefox for want of CDP, and the mutation `enter-key-not-bound` turns **only** it red. **The oracle is the paragraph count, never the revision** — the revision advanced on every broken press. Two things settled the design and both were measured: the base constructor hands `this.input` to the clipboard adapter (`editor-session.js:97`) so replacing it would split the input boundary in two, and a `<textarea>` reports Enter and Shift+Enter **identically** (`insertLineBreak` both), so no routing decision at the commit boundary can tell them apart. `findings/evidence/067/after-the-fix/` |
| **067 — the ENGINE half is still open** | `handleInsertText` increments the revision unconditionally (`src/probe_engine.cpp:2730-2755`). Now **specifiable**: `method` was measured as `paste`, so LOK accepted the newline and did nothing — the remedy is "require evidence the document changed before advancing the revision", not "handle a paste failure". **Not in any list**: it needs a relink and the payload is settled at six |
| **067 — a named uncovered path** | A `beforeinput insertLineBreak` arriving **without** a keydown (mobile virtual keyboard, autocorrect) still reaches the frozen adapter and still hits the engine no-op with a lying revision. Unmeasured, and deliberately not pre-fixed — reasoning-only fixes are how the paste double-commit was born |
| **067's original measurement** | Filed and measured tonight: revision `1 → 2`, `content.xml` **byte-identical**, typing on both sides of the key works (the control). Two source-readable candidates, both ours: the input adapter turns Enter into `commitText("\n")` instead of dispatching `insert-paragraph-break` (`input/input-adapter.js:248-251`), and `handleInsertText` increments the revision unconditionally (`src/probe_engine.cpp:2730-2755`). **Not fixed, and the reasons are named in the finding** — the adapter is finding 050's neighbourhood, the change is an interface change not a line change, one shell generation was already minted tonight, and the engine half would need a link. **Cheap next step: surface the engine's `method` field**, which already distinguishes `paste` from the `postKeyEvent` fallback and is already returned by the SDK |
...
| **068 — the caret is NOT REDRAWN after you type** (claim rewritten) | **Measured, and it is not what the finding first said.** With the repaint held still and a 24-character marker alone on its line: after typing there is **not one** caret-height run anywhere on that line; after moving the caret five left there is **exactly one**, at column 257 — and that is where it belongs (expected 254.2, one character = 9.57px). The positive control is that second read: the same scan does find a caret, so its silence in the first is absence, not blindness. Same on both the insert-field and the keyboard paths. **The operator's "before the last character" is NOT reproduced** and is recorded as unexplained. Five earlier attempts failed and each reason is in the finding — the decisive change was to stop differencing and ask directly, because the caret is ONE pixel wide (`Math.round(scaleX * 15)`) at a fractional x and never lands on an exact colour |

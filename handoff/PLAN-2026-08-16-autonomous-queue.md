...
| T2 | D3 結構輪：七份語料 × 兩瀏覽器 | **全自主** | relink（佇列完整性 #8） | 下一個 |
| T3 | D4：生命週期＋回歸 | **全自主**（Firefox PSS 取不到時判 PARTIAL，不是判過） | relink | 未開始 |
| T4 | relink 佇列補完（**寫，不連結**） | 全自主到「連結」為止 | relink | 稽核已做，四項待補 |
| T5 | D5 harness 的機器那一半 | 寫得完，**人工四格要 operator** | relink、E2-C 第二輪判定 | 未開始 |

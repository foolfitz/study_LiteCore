#!/usr/bin/env python3
"""Run R4 Provider SDK public-surface conformance in a browser."""
...
def wait_for_conformance(session: Any, timeout: float) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if status in {"action failed", "module initialization failed"}:
            raise RuntimeError(
                f"R4 conformance failed ({status}):\n{last_state.get('log', '')}"
            )
        time.sleep(0.5)
    raise RuntimeError(
        f"R4 conformance timed out with status {last_state.get('status')}:\n"
...
def main() -> None:
    ...
    parser.add_argument(
        "--url", default="http://127.0.0.1:8765/r4.html?memory=skip"
    )
    ...
    prefix = f"{args.browser}-r4-conformance"
    state: dict[str, Any] = {}
    try:

#!/usr/bin/env bash
# Finding 059, replacement predicate: native round.
#
# Prediction was committed before this ever ran:
#   findings/evidence/059/native/predicate/PREDICTION.md
#
# Usage: tools/run_f059_native_predicate.sh [OUTPUT_DIR]
...
out="${1:-$here/build/f059-native-predicate}"
mkdir -p "$out"
...
probe="$out/f059-native-format-predicate"
echo "== compiling probe =="
g++ -std=c++17 -O2 -I"$src/include" \
  "$here/tools/f059_native_format_predicate.cpp" -ldl -o "$probe"

#!/usr/bin/env python3
"""SPEC E2-B section 7: run the positive matrix and keep every saved ODT.
...
findings/evidence/sdk-e2/discovery/e2b-matrix/PREDICTION.md
"""
...
def main() -> int:
    ...
    log_text = ""
    try:
        base = f"http://127.0.0.1:{port}/e2b-matrix.html"
        wait_page(base)
        ...
        while time.monotonic() < deadline:
            metrics = evaluate(session, "globalThis.__e2b_matrix || null")
        ...
        if not metrics or not metrics.get("complete"):
            metrics = {
                "schemaVersion": 1,
                "release": "spec-e2b-matrix-matrix",
                "complete": False,
            ...
            # silently truncates.
            count = evaluate(session, "globalThis.__e2b_matrix_save_count()") or 0
            for index in range(int(count)):
                saves.append(evaluate(
                    session, f"globalThis.__e2b_matrix_save({index})"))

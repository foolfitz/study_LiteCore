...
依據兩件事：
[四輪 native 量測](../findings/evidence/queue-block-identity/README.md)，以及一份
...
| 052 的殘留呢 | 同一個 x 在行上讀 offset **4**、在文字下方讀 **7**（P-BI-6，round 4）。x 在行上被帶入、在文字下方被丟掉，所以量到的三個 x 在文字下方都落在同一個 offset。**就算真的有 block index 也沒用**——caret 確實就在最後一段裡 |
| 量到的範圍 | 三個 x、一個段落、一份語料、左至右、沒有表格。「文字下方一律丟掉 x」是這三點都飽和，左邊界與其他版面區域沒有量 |
...
**量測支持的是「缺一個針對這次輸入的回答」，不是「一定要回傳指紋」。** 對抗性審查
（2026-08-16）指得對：同樣能讓第二次同點點擊完成的契約至少還有三種——帶 request id
的 processed-event、單調遞增的 input sequence、或滑鼠事件的 completion callback。
量測排除的是「觀察狀態有沒有變」，沒有排除那三種。

選有回傳值的呼叫，理由是**它同時解掉 046**：那三種只回答「這次輸入處理完了」，而
barrier 還需要「我 dispatch 的是哪一段」。一個機制對兩個問題，比兩個機制少一個要
維護的契約——這是取捨，不是量測的結論，寫在這裡好讓之後的人可以推翻它。

這樣三個模糊都有處置，而且**不是靠比對，是靠不用再反推**——但 046 那一格帶著一條
限制，寫在第五節，**不是「一般性地解決 046」**：
...
- **兩個文字相同的相鄰段落分不出來**（P-BI-4，逐位元組相同）。指紋不是身分。
  **而這一條正好打在 046 身上**：`test-docs/e1/empty-paragraph.odt`——046 量測用的
  那份語料——結尾就是**兩個相鄰的空段落**，兩段的 `content` 都是 `""`。所以這個
  處方**涵蓋 046 已量到的那些格，不等於一般性地修好 046**。要一般性地修，需要真正
  的段落身分，也就是動 core。**這是具名極限，寫在這裡不是待辦。**
- **真正的段落身分要動 core**（`SwPosition::GetNodeIndex()` 那一類），而上游送出
  目前擱置，所以不在這條路上。
- **以上沒有一個字描述 WASM artifact。** 四輪都是 native core 26.8；shipped 引擎
  會不會看到同樣的酬載、offset 與 a11y 的更新時機是不是一樣，**要 relink 之後才
  知道**。052 的殘留「解不掉」這個結論同樣只在 native 上成立過。

...
| 基礎 | **WCAG 2.1** |
| 結構 | 4 原則、13 指引、**78 條成功準則** |
| 等級 | A（基本）／**AA（移除顯著障礙，須符合第1及第2優先等級，共 68 條檢測碼）**／AAA |
| 範圍 | 從桌機擴及**行動裝置** |

**未證實，不當事實用**：「政府機關網站一律須達 AA」沒有查到白紙黑字的法源，只查到
標章分級與檢測制度。機構採購實務盯 AA 是使用者的判斷，不是本文的量測。
...
⚠ 這是**依 WCAG 條文的判讀，不是依 68 條檢測碼的逐條比對**。判準在那份對照做完之前

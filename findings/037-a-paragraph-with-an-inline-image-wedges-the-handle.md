...
## 停等，不是空轉（2026-08-12，證據 `wedge-thread-state/chrome/`）

「呼叫不返回」有兩種，而它們要的下一步完全不同：**空轉**（跑在不會結束的迴圈裡，
CPU profile 取得到樣本，帶函式名的 build 就能指名那個迴圈）與**停等**
（停在鎖／futex／wait 上，CPU profile 一個樣本都取不到）。
Chrome 的 profiler 不必重編就能分辨——所以這一輪排在「要一個 build」之前。

擋法上線後卡死已經打不到了（那正是它的用途），所以這一輪把**存檔的 `ee185b3d`
重新打包**成診斷 profile 來重現，一樣沒有重編。

同一輪裡量兩次：卡死之前一次（控制組）、卡死當下一次。

| | 控制組（卡死前） | 卡死當下 |
|---|---|---|
| worker 0,1,2,3,7 | parked | parked |
| worker 4,5,6 | `wasm-function[36295]` 100% | `wasm-function[36295]` 100% |

**兩欄一模一樣：卡死當下沒有任何執行緒多做任何事。** 所以卡住的那條是**停等**。
`wasm-function[36295]` 的三條在卡死之前就已經 100% 了，那是這個 build 某個 thread pool 的
常態忙等，與卡點無關。

**這件事直接改掉了下一步**：不必為了「指名那個迴圈」去重編一個帶 `--profiling-funcs`
的 build——**沒有迴圈可以指名**。要問的是「它在等什麼」，那需要別的工具。

**2026-08-12 後續（引擎 `150de122`，`e2-wait-diagnostic`，出貨 artifact 未動）：**

- **那三條的名字拿到了：`__pthread_cond_timedwait`。** 帶名稱的 build 一跑就印出來，
  所以「某個 pool 在等工作」從推論變成量到的事實，`wasm-function[36295]` 這個編號也結案。
- **`-sPTHREADS_DEBUG=1` 不做我以為它會做的事。** 我挑它是因為預期它會追 futex 等待與
  代理呼叫；實際上 `libpthread.js` 裡那些 `#if PTHREADS_DEBUG` 只印**執行緒生命週期**
  （建立、TLS 初始化、entry point、清理），**沒有一行印 futex 或 proxy**。
  整輪 66 行全部來自啟動階段，卡死當下一行都沒有。
  **這是我對工具的假設錯了，不是量測失敗**——選工具之前應該先讀它印什麼。
- 所以「它在等什麼」**仍然沒有答案**，而且已知的路都走完了：profile 對停等的執行緒無效，
  emscripten 的 pthread trace 不涵蓋等待。下一個能問的層級是 **core 自己**
  （在 core 裡加 log ＝ 使用者的大編譯），或從主執行緒讀 pthread 結構與信箱狀態（未嘗試）。

**這一輪差點送出一個假結論。** 第一次跑沒有控制組，結果讀起來就是
「三條執行緒在 `wasm-function[36295]` 裡空轉」——很像答案。控制組一跑，那三條在
...
- **`getTextSelection` 裡面卡在 core 的哪一段，沒有量到。** 現在有的是「這個呼叫不返回」
  加上「它是停等不是空轉」，不是堆疊。**CPU profile 這條路已經走到底了**（見上一節）：
  停等的執行緒不會產生樣本，所以帶函式名的 build 也指不出東西來。
  要再前進得換工具——例如在 proxy 佇列與 futex wait 兩側加計數，看它在等誰。
  那要自己的診斷 build（**絕不能改出貨的 `e2-format-discovery`**）。
  **沒有它也還是可以修**——見下一節。
...
**界線**：這九種是我想得到的形狀，不是 ODF 允許的全部（沒有量 OLE 物件、圖表、
`draw:g` 群組）。**「沒有反例」不等於「不存在反例」。**

**2026-08-12 補量了原本列在這裡的兩個情境，其中一個是反例：**

| 情境 | 結果 |
|---|---|
| **表格儲存格裡的 as-char frame** | 選取型別 `complex`，**走現有擋法**，正常拒絕 |
| **註腳本文裡的 as-char frame** | **卡死，而且擋法擋不到**——見 [038](038-a-frame-inside-a-footnote-wedges-the-engine-on-selection.md) |

**038 是這個擋法的涵蓋範圍之外**：它的卡死發生在一次**滑鼠拖曳選取**之後，
那條路上根本沒有 barrier 的讀取可以擋。**擋法沒有壞，但它不覆蓋那裡**，
所以「含 as-char frame 的文件」在產品面**仍有一個開著的洞**。

另外，`char` 錨定這一格現在知道得更細了（見下）：它讀回 `complex`、被擋法拒絕，
**但它的 close 不卡**——所以擋法對 char 錨定**可能是過度拒絕**，而那還沒量過
（要在無擋法的 artifact 上跑才知道 html 讀取會不會真的卡）。

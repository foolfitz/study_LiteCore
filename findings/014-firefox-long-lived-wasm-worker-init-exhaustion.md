...
### 待驗證 10 結案：那不是殘留，是回收延遲（已觀察，2026-08-08 第四次）

一輪同時做完兩件事——把單頁世代數推到 **100**，並在最後一代之後讓頁面**閒置 240 秒**
繼續取樣（`--idle-tail-seconds`，樣本另標 `checkpoint=idle-tail`，不進正式閘門）：

| | 50 代（stock） | 50 代（`maxPerDomain=64`） | **100 代** |
|---|---|---|---|
| cycle | 50/50 | 50/50 | **100/100** |
| block 數 | 4 | 4 | **9** |
| robust 斜率 | 22.4 MB／block | 25.9 MB／block | **6.61** |
| 首末成長 | +57.4 MB | +70.3 MB | **−23.8 MB（−1.68 %）** |

100 代的 block 中位數（MB）：
`1411.7 / 1393.0 / 1402.0 / 1430.0 / 1440.2 / 1487.5 / 1477.3 / 1358.2 / 1387.9`
——**升到 1487 之後掉回 1358（單一 block 內 −119.1 MB）**，首末淨成長是**負的**。

**兩輪 50 代看到的「單調上升」是視窗太短的假象**：4 個點看不出振盪，9 個點就看見了
那次回收。（第三次複核已經算過「4 點單調的隨機機率 1/24 ≈ 4 %」——現在知道那不是
低機率巧合，而是 4 點根本不足以分辨上升與振盪。）

**閒置尾段更直接**（233 個樣本、239.6 s）：整棵行程樹**穩定在 624.6 MB**，比最後一個
block 中位數（1387.9 MB）**低 763.3 MB**，且在前幾個樣本內就掉下去、其後 235 秒完全平坦
（軌跡 625.9 → 624.7 → 624.6 → 624.6 → 624.6 → 624.6 → 624.6）。

**結論（已觀察）：跑動期間的 post-close 讀數反映的是「回收落後於工作量」，不是累積。
工作一停就回到 ~625 MB。**因此：

- **「每代殘留 X MB」這個量本身不存在。**我 2026-08-08 第二次寫的 1.4～2.6 MB／代與
  第 207～374 代、以及第三次修正後的 1.9～2.6 MB／代與第 207～281 代，
  **全部撤回**——第三次修對了我的分母，但那個被除的量是假象。
- 產品端**沒有任何已量測的記憶體理由**要求限制每頁世代數。
- 單頁實測深度由 50 提高到 **100**（Firefox 153.0.1）。

證據：`findings/evidence/sdk-r7/single-page-generations-100/firefox/s2-fresh/run-1/result.json`。
...
|---|---|---|---|
| `s1` run-1／2／3 → `s2-reuse` | 49865 → 49977 | 12.5／12.4／12.6 s | **＝harness 的 session churn 常數（2026-08-08 第四次實測重現 12.8／12.5／12.4 s）** ⇒ **同一次 `--all` invocation、同一條 pipe** |
...
> **2026-08-08 第四次更正**：第一列原本寫「正好是 runner 的 `time.sleep(12)`」，**這是錯的**。
> 那個 `sleep(12)` 只在 `run_firefox_fresh_batched()` 的批次之間，`s1`／`s2-reuse`
> **根本不走那條路徑**。改用實測認定：重跑同一段序列，run 間隔量到
> **12.8／12.5／12.4 s**（`s1` 本身只跑 3.7 s，其餘是 session 拆除＋geckodriver／
> Firefox 冷啟動），與 08-04 的 12.5／12.4／12.6 s 幾乎重合。
> 所以 12 s 是**已量測、可重現的 harness churn 常數**，763 s 與 97 s 是它的 6～60 倍——
> invocation 邊界的認定因此成立，而且比原來的（讀錯程式碼的）理由更硬。
...
| 單頁不分批 | **非空，實測 10,033 B**（接在 `s2-reuse` 後 12 s） | 10,033＋877＋34×1614 ＝ **65,786 B** | **超出 250 B** | **34** |
...
之後，共用同一條已經裝了東西的 pipe。**這個前綴已於 2026-08-08 第四次直接量到
（已觀察，待驗證 9 結案）**：

用 `--plan-prefix 4`（＝`FORMAL_PLANS[:4]`，正好 `s1`×3＋`s2-reuse`）走**同一段產品程式碼**
重跑，`serve.py` 的輸出導到一個真檔案（不是 pipe），逐輪取檔案大小：

| 取樣點 | 累計 B | 增量 B |
|---|---|---|
| `wait_page()` 之後（尚未跑任何情境） | 77 | 77 |
| `s1` run-1 | 2,562 | 2,485 |
| `s1` run-2 | 5,047 | 2,485 |
| `s1` run-3 | 7,532 | 2,485 |
| `s2-reuse` run-1 | **10,033** | 2,501 |

模型要求的前綴是 **9,783 B**，實測 **10,033 B**——差 250 B，約 **2.5 行 log**
（101 行／10,033 B ⇒ 每行 99.3 B）。把實測前綴代回去：

- 第 **33** 個 cycle 結束時累計 **64,172 B**，距 64 KiB 還差 1,364 B ⇒ **cycle 33 完成**；
- 第 **34** 個 cycle 只走了 1,364／1,614 B（**85 %**）就撐破 65,536 B ⇒ **牆正好落在 cycle 34**。

實測牆就是 34。**前綴是獨立量的、不是回頭湊的**，所以這是一次真正的樣本外命中，
第三列因此從「與模型相容」升格為**獨立佐證**。三種分批形狀現在全部由同一組
（877 B／導覽、1614 B／cycle）＋各自的起始存量解釋，沒有任何一個自由參數被留下來吸收誤差。
證據：`findings/evidence/sdk-r7/serve-log-prefix/`（`serve.log` 原始檔＋
`firefox/summary.json` 的 `serverLogLedger`）。
...
8. **（新）下游規格的限制要不要撤**——見〈下游影響〉。每一條都要各自重跑對應矩陣才能撤，
   ...
   之前先看那張表。
9. ~~**（新，2026-08-08 第三次）量 `s1`×3＋`s2-reuse` 的 `serve.log` 位元組量**~~
   **已完成（2026-08-08 第四次）：實測 10,033 B，模型要求 9,783 B，差 250 B（約 2.5 行 log）。**
   代回去把牆算在 cycle 34 的 85 % 處，實測牆就是 34。見〈位元組帳〉。
   附帶結果：`s1` 之間的 run 間隔實測 12.8／12.5／12.4 s，與 08-04 的 12.5／12.4／12.6 s
   重合，**同時推翻「那 12 s 是 `time.sleep(12)`」的說法**（該 sleep 不在這條路徑上）。
10. ~~**（新，2026-08-08 第三次）分開「記憶體殘留」與「回收延遲」**~~
   **已完成（2026-08-08 第四次）：是回收延遲。**單頁 100 代 100/100，首末淨成長
   **−23.8 MB**；閒置 240 秒後整棵樹落在 **624.6 MB**，比最後一個 block 中位數低
   **763.3 MB** 並保持平坦。所有「每代殘留 X MB」與由它外推的上限**全部撤回**。
   見〈待驗證 10 結案〉。原文如下——
   ~~在最後一代之後
...
- **2026-08-08（第四次）：待驗證 9 結案，位元組帳三列全部閉合。**
  用新的 `--plan-prefix 4`（走產品程式碼，不是另寫一份）＋`OXSDK_SERVER_LOG_DIR`
  （寫真檔，不是 pipe）量 `s1`×3＋`s2-reuse` 的 `serve.log`：**10,033 B**，
  模型要求 9,783 B，差 250 B。代回去牆落在 cycle 34 的 85 % 處，**實測牆就是 34**。
  前綴是獨立量的，因此是樣本外命中，第三列升格為獨立佐證；**模型不再有任何自由參數**。
  同時更正時間軸表第一列：12 s **不是** `time.sleep(12)`（那個 sleep 只在
  `run_firefox_fresh_batched()` 的批次之間，`s1`／`s2-reuse` 不走該路徑），
  改以實測的 session churn 常數 12.8／12.5／12.4 s 認定 invocation 邊界——
  理由換掉，結論不變且更硬。

...
### 同日稍後的另一個更正：`changed` 是沒有檢查支持的宣稱

第一版 readback 實作照舊送 `"changed":true`。readback 回答的是「文件現在是不是目標狀態」，
**不是「是不是這次派送把它變成這樣」**——重複按下時文件本來就對，`changed:true` 就是假的。
規格 v10 早就決定不宣稱這件事，程式沒跟上。已改 `"changed":null`，兩瀏覽器重跑
9/9 完成、`changed` 全 null（WASM `80b48abf…`）。

...
> **修法對「正常選取」是量到的零影響**：沒做格式動作的控制臂在修前修後
> 四個欄位加延遲完全相同（兩瀏覽器）。前後對照與界線寫在
> [`no-op-for-normal-selections.md`](no-op-for-normal-selections.md)
> （依 `AGENTS.md`，那一份是英文）。
...
> **後續（同日）**：上面兩句是**這一輪**的界線，仍然成立。
> 重掃已於同日執行並通過（[`a3a4a5-940b7723/`](a3a4a5-940b7723/README.md)），
> 六項撤限門檻逐項達成，**縮限 4 已於 SPEC E2-A v23 撤除**。
> 記在這裡免得有人只讀這一份就以為重掃沒跑。

...
成功的臂比失敗的臂多 **1 次** `LOK_CALLBACK_TEXT_SELECTION`——多的就是 `END` 那一次，
失敗的臂在 `END` **沒有廣播**，因為選取確實沒有改變。

> **就地更正（2026-08-15，第二輪複驗抓到）**：這一段原本寫「成功的臂有 **2 次**、
> 失敗的臂只有 **1 次**」。**那兩個絕對數字不穩定**——[第二輪](../native-round2/README.md)
> 用同一支探針、同一段量測程式碼重跑 A～F，選到與否六臂全同，
> 但計數變成成功 1 次／失敗 0 次，**六臂一致差 1，我無法歸因那個差**。
> 可引用的是**同一輪之內的差**，不是絕對值。上面已照這個改寫。

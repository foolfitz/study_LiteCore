...
| 觀察到的 | 判定 | 要做的事 |
|---|---|---|
| （A）出現 `IdlesLockGuard`，且經由 `getSelection` 系呼叫下的 `SwTransferable` 解構抵達 | **預測成立**：038 是同一缺陷的第三個入口 | 040 的上游單增列第三個入口；038 檔案改判並連到 040 |
| （B）出現 `IdlesLockGuard`，但抵達路徑不同（例如不經 `SwTransferable`） | **同一缺陷、不同路線** | 上游單照實寫路線差異，**不可把 A 的敘述套上去** |
| （C）停在別的等待原語或別的條件 | **038 是另一個缺陷** | **040 的上游單不得宣稱 038**；另開 finding |
| （D）這顆 artifact 上重現不出卡死 | **什麼都沒量到** | 照實記「未重現」，**不准事後解釋為什麼**；038 維持「推論、未取得堆疊」 |

（D）要特別提防：它看起來最像「沒事」，而它其實是**這一輪失敗**。

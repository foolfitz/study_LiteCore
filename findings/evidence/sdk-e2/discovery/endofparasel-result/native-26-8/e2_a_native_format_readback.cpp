...
void onCallback(int type, const char *payload, void *) {
  // Finding 033's remediation hinges on whether .uno:EndOfParaSel returns a
  // UNO command result -- that result is the only thing that could attribute
  // the selection the barrier waits for.  A2 measured the five format commands
  // and never this one, and "these commands return no result" is exactly the
  // belief native measurement overturned on 2026-08-05, so it gets measured
  // rather than reasoned from init.cxx's generic notify path.
  if (type == LOK_CALLBACK_UNO_COMMAND_RESULT)
    std::cout << "{\"stage\":\"uno-result\",\"atMs\":" << elapsedMs()

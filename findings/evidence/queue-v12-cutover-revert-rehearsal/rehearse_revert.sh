...
NEWGEN=e2/editor-shell-v2-bundle-v44.json
...
python3 tools/build_e2_c_shell_bundle.py --manifest "$NEWGEN" \
        --frozen-date 2026-09-06 --write > "$SP/freeze-v44.json" 2>&1
echo "freeze exit=$?  (what matters is the next line)"
python3 "$SP/check_written.py" "$SP/freeze-v44.json" || { echo "STEP 3 DID NOT WRITE"; exit 1; }
python3 "$SP/repoint_manifest.py" v43 v44 || { echo "STEP 3 REPOINT FAILED"; exit 1; }
grep -n '^MANIFEST = ' tools/build_e2_c_shell_bundle.py

echo
echo "=== RUN THE NET ON THE CUTOVER PAGE (no --candidate-profile) ==="
python3 tools/run_e2_c_product_path.py --out "$SP/rehearsal-after-cutover.json" \
    > "$SP/rehearsal-after-cutover.log" 2>&1
echo "runner exit=$?"
python3 tools/check_usable_editor.py --report "$SP/rehearsal-after-cutover.json" \
    > "$SP/reconcile-after-cutover.json" 2>&1
...
python3 tools/run_e2_c_product_path.py --out "$SP/rehearsal-after-revert.json" \
    > "$SP/rehearsal-after-revert.log" 2>&1
echo "runner exit=$?"
python3 tools/check_usable_editor.py --report "$SP/rehearsal-after-revert.json" \
    > "$SP/reconcile-after-revert.json" 2>&1
echo "reconciler exit=$?"

...
| [037](037-a-paragraph-with-an-inline-image-wedges-the-handle.md) | 對「含行內圖片的段落」派送封閉格式動作會**卡死 document handle**，而 034 加的 5 秒 per-stage deadline 沒有接住 | **已確認（`ee185b3d` 與 `38168306` 各一次，行為一致）／未修**：動作死於用戶端 20001 ms 逾時而非 5000 ms 的 `stage-deadline:*`，接著 `search` 30 s 逾時、`handleUsableAfter=false`、之後每一列都死在 `getState` 逾時；文件本身仍關得掉（`closeMs: 10906`），所以卡死的是**操作**不是 worker。`PC-IMAGE` 是 M2 二十一種形態裡**唯一 `selectionType=3`（`LOK_SELTYPE_COMPLEX`）**的一列，其餘 20 列都是 1，而 barrier 的判定**完全沒有讀 selection type**；原生 26.8 讀得回 markup（798 bytes、`<img/>` 巢狀在 `<p>` 內、錨點文字在），**所以問題不在序列化器而在 wasm 側的階段推進**。**不是本次改動造成的**：把 `build/archive/e2-format-discovery-38168306/` 換回 `dist/profiles/` 跑同一份 app 同一批 21 列，同一列同樣 20001 ms 逾時——既存缺陷，只是新 fixture 才讓它被看見（兩份證據各附 `ARTIFACT.sha256`）。**375 次判定派送沒碰到**的原因與 [034](034-paragraph-selection-escapes-at-the-offset-the-test-never-used.md)／[035](035-the-postcondition-read-fails-closed-on-any-formatted-or-cjk-paragraph.md) 同形：既有 fixture 沒有一個被派送段落帶行內圖片（`r7-t2-styled.odt` 有圖片但從不是派送目標），覆蓋軸沒進矩陣——同一個病第三次換軸出現。**方法論代價已記錄**：它第一次排在第 16 列，卡死後**連累後面五列全部未量測**，其中 `pc-footnote` 正是 (a′) 裁決所依賴的那一列；移到最後重跑才取得那 20 列。**原生重放已縮小範圍（`barrier-replay/native-26-8/`）**：把 barrier 的階段照順序在原生重放、六個錨點一起跑，**每一階段都 ≤10 ms，含 PC-IMAGE**（COMPLEX 選取讀回 798 bytes、還原 10 ms 清空、handle 事後可用）——**所以「LOK 對 COMPLEX 選取做了什麼」不是成因**，問題在 WASM 引擎的階段機器或事件推進。該輪自己抓到一個假訊號：第一版用同一個錨點再搜一次測可用性，六列裡五列回 false **含控制組 PC-PLAIN**——那只是游標已在唯一命中處之後，改搜別的錨點後六列全 true（**在控制組上也會亮的訊號不是訊號**）。**仍未量**：WASM 側卡在哪個 stage（重放等的是「任何 UNO result」且不在引擎主迴圈裡跑，只證明序列在 LOK 層做得完，不證明引擎跑的是同一串）、其餘四個封閉動作、Firefox、上游重複單。**下一步要一個診斷用 WASM profile**（stage 在武裝與逾時各印一次＋引擎迴圈是否還在轉），**必須是自己的 profile 不能是出貨的那個**；5 秒 deadline 完全沒作用本身是線索——迴圈沒轉就沒有東西去武裝或檢查 deadline，那樣卡點在 barrier 之上 | **嚴重** | 否（我方 barrier；COMPLEX 選取本身是 core 行為） | — |

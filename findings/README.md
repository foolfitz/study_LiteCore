...
| [035](035-the-postcondition-read-fails-closed-on-any-formatted-or-cjk-paragraph.md) | 後置條件讀取對「有字元格式或含中日韓文字」的段落一律 fail closed | **已確認（兩個 build × 三個 fixture 實測，行為一致）／未修，修法待決**：`formatTagIsKnown` 只收 `ul/ol/li/h1/p`，遇到集合外任何標籤就 `unknownTag=true` 並中止，`formatBarrierReadbackSatisfied()` 第一行即回 false。而序列化器對兩種**極其普通**的段落都會寫出集合外標籤：`Normal **bold anchor**…` → `<b>`／`<i>`；**`第三段跨行 gamma` → `<font face="Noto Sans CJK KR"><span lang="zh-TW">`**——**只要段落含中日韓文字，序列化器就會為那段換字型 run**，這不是特殊格式而是中文文件的常態，**產品面可能比 034 更重**。**375 次判定派送一次也沒碰到**：A3／A4／A5 的錨點段落（`E1-STYLED-END`／`E1-MULTI-END omega`／`E1-PLAIN-END`）全是純 ASCII 且所在段落沒有任何 run——fixture 裡明明有中文，但**沒有一段是被派送的那一段**。與 034 同形狀、換一個軸。**不是想到的，是 034 的修法逼出來的**：修法後 barrier 開始讀對的段落，同一案例仍失敗，第一反應是「修法造成退步」；用 offset `Len()`（不可能逃逸）重跑同一段，**兩個 build 都失敗、`unknownTag` 都是 true** → 既有缺陷。該列現在留在鑑別器裡叫 `inline-formatting-readback`，就是為了讓兩個原因不能再被混為一談。修法選項三條已列（傾向：把「影響判定的標籤」與「段落內可出現的標籤」分開，行內標籤略過不計而非加進已知集合，否則 `multiBlock`／`blockTag` 計數會被污染）。**未併入本次 build**：混進同一個 artifact 會讓兩個改動都無法對應到自己的證據（[027](027-verdicts-must-be-recomputed-against-the-artifact-that-produced-them.md)），這是下一個 build 加下一次重掃 | **嚴重** | 否（我方 readback 的封閉標籤集） | — |

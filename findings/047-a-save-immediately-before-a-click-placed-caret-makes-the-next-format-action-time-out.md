...
## 修訂 2026-08-16：這個順序在現行殼層上跑不出來了（**不是**判定它修好了）

產品路徑 harness（`tools/run_e2_c_product_path.py`）需要一個能把佇列擋住的失敗，
好去驅動產品的復原按鈕；047 是這棵樹裡唯一有記錄、可從產品 UI 走到那個狀態的
路線。**兩輪都沒有重現**：`儲存 → 點擊放游標 → 項目符號`，三個動作在頁面內同一次
呼叫裡背靠背送出（沒有 WebDriver 來回、沒有輪詢），結果是
`• 項目符號 28 ms`、修訂號前進、狀態留在 `ready`。

**這不足以說 047 修好了**，理由三個：

1. 047 是 **2026-08-15** 在**另一個 harness**、**048 修法之前的殼層**上量的，而
   048 改掉的正是 `placeCaret` 等什麼——047 自己的診斷就是「`placeCaret` 的確認
   不足以保證下一個動作會成功」。**同一句話的兩面。**
2. 這兩輪的目的不是量 047，是要走到別的狀態；沒有對照組（例如刻意用舊殼層跑同一
   個順序）。
3. 只有 Firefox，只有一份語料，只有兩次。

要判定它，需要它自己的一輪：兩個殼層代、兩個瀏覽器、四組對照照原樣重跑。
佇列項 `queue-047-may-have-closed-under-048`。

...
| | |
|---|---|
| **狀態** | **已修（2026-08-19，殼層 v21，不需 relink）** |
...
## 修法：已修（2026-08-19，殼層 v21）

產品頁改用 `recoveryNotice(snapshot)`，不再自己重新推導。三分支照殼層的判斷走，
「寫入失敗」有自己的句子：

> 引擎需要重新開啟。我們試著先保住你的工作，那次存檔失敗了——自上次儲存以來的內容不會回來。

**順帶收掉 finding 054 的根源**：按鈕的 `disabled` 現在來自同一個模組的
`restartPossible`，不再讀一個沒有人寫的欄位。**054 和 061 是同一次重新推導生出來的
兩個缺陷**——這就是「用那個模組，不要抄它的邏輯」最好的論據，也是外部裁決當時指出的。

判準也換了：產品把渲染出來的判斷蓋在 `#notice[data-rescue]` 上，檢查要求**狀態列的
藥丸與通知這兩個介面一致**，而不是比對措辭。比對措辭的版本會在別人改一次句子時
**靜靜變綠**——那是同一次裁決在我的檢查裡找到的三個洞之一。

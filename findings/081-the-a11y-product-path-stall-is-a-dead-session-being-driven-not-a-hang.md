...
| | |
|---|---|
| **狀態** | **處方 1 已做並量到它生效**（2026-08-26）。**第一個失敗的成因已確立：4/4 都是同一格同一支臂**，見下與 finding 082 |
...
**後續（同日）**：另外三輪跑完了，**4/4 完全一致**——同一格、同一支臂、
`recorded: 17/40`、`pending: 0`、`checkpoint: 無`。型別化 payload 也收到了，
確立臂 2 那次**閘門真的跑了、兩次讀都成功、讀回是單一 block、containment 成立**，
而兩個指紋仍然不同。已另立 **finding 082**。
指紋本身沒有被吐出來（payload 不帶），所以「讀回的是哪一段」仍未確立。
...
- 2026-08-26：處方 1 實作完成並量到它在真的死掉的 session 上生效（v9，5 分 22 秒停下、
  ...
  同日稍後再跑三輪：**4/4 完全一致**，並收到 barrier 的型別化 payload——
  臂 2 那次閘門真的跑了、兩次讀都成功、讀回是單一 block、containment 成立。
  已另立 **finding 082**；「讀回的是哪一段」仍未確立（payload 不帶指紋）。

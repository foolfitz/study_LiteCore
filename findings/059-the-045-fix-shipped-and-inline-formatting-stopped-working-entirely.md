...
| | |
|---|---|
| **狀態** | **已修並出貨（2026-08-19 連結，artifact `296f3ea727725fbb`）** |
...
## 修法：已出貨（2026-08-19 連結，artifact `296f3ea727725fbb`）

引擎不再拿核心的 `success` 當判準，改呼叫 `inlineFormatArgumentResolved()`——
**比對觀察到的狀態與要求的狀態**，而且要求狀態是**已知的**（`…Known`），
所以「我不知道」永遠不會被讀成「成功」。

一併修掉的：

- **底線與刪除線現在有 state cache 了**。它們一直都在核心的 `GetKitUnoCommandList`
  裡，而引擎裡那句說它們不在、因此不留 cache 的註解是假的——原始碼與實測都反證過。
- 那句假註解換掉了。

**用 `OXSDK_E2_FORMAT_BARRIER` 包起來**：每個 profile 共用這段程式，而 E1-C 在特定
WASM hash 上驗過那些凍結 profile，所以行為改動只給需要它的那一個。

**是 runner 自己說這件事修好了**：`a-format-that-worked-is-not-reported-as-failed`
從 KNOWN_RED 變綠，而 `finish()` 把「宣告了紅卻通過」當成**過期宣告**並讓整輪失敗。
不是我去確認的，是它不肯讓那個宣告活過缺陷。

...
### 9.5.9 D3 執行完畢，但先掉出 [finding 048](../findings/048-place-caret-confirms-before-the-click-takes-effect.md)（2026-08-15）

D3 的八格第一次能派送的那一輪**整輪作廢**，而作廢的理由不是清單：**八格全部在
文件第一段動作**。`set-list-unordered` 把 `E1-LC-HEADING` 變成清單項，而那一格的
錨點是四段之外的 `E1-LC-BETWEEN`；三格 `set-list-none` 打在本來就不是清單的標題
上，回報 **`verified-format-readback`**。

根因是 `EditorSession.placeCaret` 在點擊生效之前就回報成功——它的判準是「有一個
收合的、observed 的游標」，而那在文件剛開啟時就已經成立。點擊本身要 **22–28 ms**
才落地（輪詢量到，八格無一逾時）。
...
| | |
|---|---|
| **harness** | 每一格在派送**之前**讀回游標矩形，不在錨點就**拒絕派送**，並且離線判定器**自己重算**一次而不採信頁面的布林值。第一次寫這個閘門時要求了一個引擎根本不會發出的 `available` 欄位，於是連好的那一臂也一起擋掉——閘門本身也要能被證明會放行 |
| **產品** | 未修。`web/e2-editor-app.js` 的 canvas 點擊走的就是這條路。看起來現成的解是改用 E1-D 已經為範圍選取硬接的 `setTextSelection` 零寬選取（本輪八格立刻命中），但那是**產品**的修法，要不要進、什麼時候進，另外判 |

**D2 沒有被作廢，而這是量出來的不是推出來的**：D2 每一格在手勢與動作之間都有一次
`snapshot()` 存檔，約 250 ms 遠超過點擊需要的時間，而 `d2-refused-no-mutation-engine`
的存檔文件顯示變成清單項的**正是點的那個空段落**。finding 048 的第一版寫成「點擊
完全無效」，就是被這份文件推翻的。D2 仍然依 048 改成可證位置的路徑並重跑，
**12 格中 11 格通過**；`d2-refused-no-mutation-engine` 現在落在
`MUTATION_OUTCOME_UNKNOWN`／`multi-block-readback`（游標真的在空段落上時），
與 9.5.8 記的 `postcondition-not-met`（游標在第一段時）**是兩個不同的出口**，
兩者都指向 [046](../findings/046-an-empty-readback-is-reported-as-the-document-being-in-the-wrong-state.md)
還沒修的那個分類問題。**046 的修法目前不在原始碼樹裡**——本輪查證時發現，
`empty-readback` 這個 shape 從來沒有被寫進 `probe_engine.cpp`；relink 佇列要據此更正。

#### 八格的結果：**六個預測成立、兩個不成立、零內容遺失**

判準是 `PREDICTION.md` 在 harness 存在之前登記的那八條，逐條由
`tools/analyze_e2_c_d3.py` 離線施加：

| 格 | 預測 | 實測 | 成立 |
|---|---|---|---|
| L1 | 獨立段落 → 新的一項項目符號清單，前後不動 | 就是如此 | ✅ |
| L2 | 最後一段 → **新的**編號清單 | **併入上面那個編號清單**（3 項） | ❌ 行為可接受，預測不成立 |
| L3 | 第二項離開清單，第一項留著 | 就是如此 | ✅ |
| L4 | 同上，編號版 | 就是如此 | ✅ |
| L5 | 第一項就地轉成編號，**不出現第三個清單** | 轉成編號了，但**清單從 2 個變 3 個** | ❌ 行為可接受，預測不成立 |
| L6 | 重複派送 → `<office:body>` 位元組相同 | **與「什麼都不做的存檔」逐位元組相同** | ✅ |
| L7 | 中間項離開 → 清單一分為二、**不掉項** | 一分為二、三項都在、第二段帶 `text:continue-numbering="true"` | ✅ |
| L8 | 與前面的項目符號清單**合併**，後面的編號清單維持獨立 | 就是如此 | ✅ |

**L6 的判準在執行時就地更正過一次，理由要記下來**：預測寫的是「`<office:body>`
位元組相同」，而第一次拿它去比**磁碟上的語料**——那必然不同，因為語料是本 repo
的產生器寫的、存檔是 LibreOffice 的 ODF 匯出寫的（sequence-decls、樣式名、屬性
順序）。能回答那個預測的比較對象是**同一份文件、什麼都不做的存檔**，所以本輪加了
一格 `L6C` 控制格。**這是判準的更正，不是判準的放寬**，而且它讓 L6 從「無法量」
變成「量得到而且成立」。

**L7 是上位規格停止條款正對著的那一格**（SPEC E2-000 第 10 節：清單切換造成無聲
的 ODT 結構遺失）。**沒有觸發**：三項都在，編號延續。八格全部 `textLost: []`。

兩個不成立的預測是同一種東西：**相鄰**。L2 併入了相鄰的同型清單、L8 也是；L5 因為
轉成了**不同型**，所以必然分裂出新的清單。事後看是一條連貫的規則——「加入相鄰的
同型清單，沒有就自己開一個」——但**預測是照寫下來的樣子計分的**，「我們本來也會
接受」是要在跑之前決定的事。

#### 三輪交叉核對

八格 ＋ 控制格共九份文件，在三輪之間**逐位元組相同**：`range-v2` 與 `run-2`
（零寬 `selectRange`）以及 `click-poll`（產品手勢，輪詢到游標抵達）。兩種不同的
定位手勢、三次執行，同一個結果。


#### 正式輪：產品手勢 ＋ 可證的落點（run-3）

上面那張表是用零寬 `selectRange` 定位跑出來的，而 9.5.6 早就記過**手勢會改變
答案**。所以正式輪改成「產品的點擊 ＋ 輪詢到游標抵達才派送」，兩個瀏覽器各一輪
（`d3-lists/run-3/`），共用的定位工具是 `web/e2-c-caret.js`
（**刻意放在 `editor-shell-v2/` 之外**：殼層 bundle 綁的是產品模組，harness 的
工具不該動那個雜湊——實測 bundle digest 未變）。

**結果逐格相同**，而且更強的一句是：`run-2`（selectRange，雙瀏覽器）、
`run-3`（產品點擊，雙瀏覽器）與 `caret-probe/click-poll` 五輪之間，
**九份存檔文件的 `<office:body>` 逐位元組相同**。游標抵達時間：
Chrome 22–28 ms、Firefox 23–33 ms。

#### D2 的空段落格：**手勢會改變引擎的分類**

同一顆 artifact、同一個空段落、同一個 `set-list-unordered`：

| 游標怎麼來的 | 回報 | session |
|---|---|---|
| 裸的 `placeCaret`（游標其實是在之後的存檔期間才落地） | `EDITOR_FORMAT_POSTCONDITION_FAILED`／`postcondition-not-met` | `ready` |
| **派送前已證明游標在空段落上**（產品點擊＋輪詢，或零寬 `selectRange`） | `MUTATION_OUTCOME_UNKNOWN`／`multi-block-readback` | `recoverable-error` |

兩者 `postBlocks` 都是 0，而 `multiBlock` 的定義是
`blockCount > 1 || itemCount > 1`——**所以第二列是 itemCount 判的，而產品投影
根本沒有 itemCount 這一欄**。這是 [046](../findings/046-an-empty-readback-is-reported-as-the-document-being-in-the-wrong-state.md)
的更正段，也是 relink 佇列新增 3c（把 itemCount 補進產品投影）的理由：
**沒有那一欄，「零個 block」分不出「什麼都沒讀到」與「只讀到清單項」，
而 3b 的判準正壓在這個區別上。**

D2 的判定表因此就地改成量到的那一對，**並且明寫它是看過結果之後才寫的**
（`tools/analyze_e2_c_d2.py` 的註解）。同時修掉這一格的一個 harness 缺陷：
派送後失敗會擋住佇列，而 `save` 正是被擋的操作之一，所以 after 圖必須在**規定的
recovery 之後**才取——這一格為此連兩輪死在 `EDITOR_NOT_READY`，關於引擎什麼都沒說。

修完之後的 `d2-narrow/run-4/`：**兩個瀏覽器各 12／12 全過**。看過結果才寫的那一對
在**它沒看過的那一輪（Firefox）上成立**，這是它現在唯一的憑據——不是它被寫下來的
方式好，而是它在之後的一輪上活下來。這一格現在斷言的不是「什麼都沒變」（動作確實
派送了、結果未知），而是**規定的 recovery 把文件放回去了**：rollback 回 `ok`，
而 `<office:body>` 在 rollback 之後與 before 圖逐位元組相同。判定器的自我測試也補了
兩條，證明這個新檢查會說不。


#### 產品的點擊已修（2026-08-16）

`EditorSession.placeCaret` 現在要「引擎確認了這次點擊」**且**「游標在點的那一
行」才返回；安靜滿 400 ms 之後才接受一個已經在那一行上的游標（同點點擊，
引擎沒東西可說）。**沒有改接 `setTextSelection`**——那會保住 E1-C 的綁定、
也會讓產品的點擊手勢永遠沒被驗證過，而那正是 048 的成因。

D3 與 D2 在修好的殼層上兩瀏覽器各重跑一輪（`d3-lists/run-4-fixed-shell/`、
`d2-narrow/run-5-fixed-shell/`）：D3 八格與修法之前**逐位元組相同**，
產品自報等待 22–28 ms、八格全部 `engine-acknowledged`；D2 兩瀏覽器各 12／12。

**代價已申報**：`editor-shell/editor-session.js` 是 E1-C 殼層 bundle 綁的五個
之一，`E1_GO_ODT_EDITOR` 的殼層綁定因此斷開（SPEC E1-C §11.7）。E2 的殼層
...
——而那一輪正是 [047](../findings/047-a-save-immediately-before-a-click-placed-caret-makes-the-next-format-action-time-out.md)
寫出來的那一輪。已從 git 還原，被覆蓋的那一輪另存為 `run-1b-before-skip/`。
`tools/run_e2_c_d0.py` 現在會在**啟動瀏覽器之前**檢查目標目錄有沒有 `result.json`，
有就拒絕執行（`--overwrite` 才會蓋）。**一輪就是一筆紀錄；第二輪要有自己的目錄。**

### 9.5.10 D3 的語料半邊執行完畢（2026-08-16）：**八格全過，一條預測不成立，而且掉出一個語料缺陷**

清單八格之外的另一半：**在指名錨點做一次普通編輯，文件其餘部分要原封不動**。
七份語料裡兩份（`list-contexts`／`list-split`）依矩陣的措辭是「carrying」L 格，
所以結構判準**離線施加在 L 輪已經存好的文件上**，不重跑瀏覽器；其餘五份跑八格
（`l0-t3` 首／中／末、`l4-stress-100` 第 50／100 頁各一格）。動作全部是
`set-list-unordered`——一個動作跨七份語料，差異才歸因得到文件而不是動作。

**結果（`run-3-with-controls`，兩瀏覽器）**：八格全部派送、游標 22–29 ms
`engine-acknowledged`；必要錨點逐頁保住（22 頁與 100 頁的**每一頁**錨點都數）；
結構與控制格相同；桌面重開與 PDF 全過；carried 兩份也全過。

**一條預測不成立**：P-C5（兩瀏覽器 `<office:body>` 逐位元組相同）在
`c-l1-review` 上不成立。量到的原因是**自動產生的追蹤修訂 ID**
（`ct311…` 對 `ct2888…`）——兩份長度相同、可見文字相同、把 ID 正規化之後逐位元組
相同。**照原樣記為不成立**：「我們本來也會接受」是要在跑之前決定的事（L2／L5
同一條規矩）。更窄的判準（正規化後相同）進第二輪矩陣。

#### 掉出來的東西不是產品缺陷，是**語料缺陷**

`l4-stress-100` 的兩格第一次判定時 `structurePreserved` 紅：語料裡 100 個
`<draw:frame>`／`<draw:image>`，存出來**零個**，ZIP 裡的 `Pictures/` 也不見了，
檔案從 35,772 掉到 14,977 bytes。這是停止條款的形狀，所以在下任何結論之前先做對照：

| 對照 | frames 之後 |
|---|---|
| 系統 LibreOffice 26.8，`--convert-to odt`，**完全沒有編輯** | **0** |
| `build-native-26-8`（WASM profile 的同一顆 core commit），同上 | **0** |

**兩個 LibreOffice 都不留它，而且過程裡沒有任何編輯器。** 原因在標記本身：
每一個都是 `<draw:frame text:anchor-type="as-char">` **直接掛在 `<office:text>`
底下**，是 `text:p` 的兄弟而不是在段落裡；as-char 卻沒有可依附的字元，匯入濾鏡
不留它。E1-C 在 2026-08-13 已經記過一半（「屬性 101、有效 1」），**沒記到的是
那 100 個連存檔都活不過**。

因此判準改成「結構與**同一份文件什麼都不做的存檔**比較」，每份語料加一個控制格。
**這是判準的更正不是放寬**——與 L6 學到的是同一件事，只是規模大得多：不加控制格，
每一個碰到那份語料的格都會把語料自己的缺陷報成編輯造成的內容遺失。

#### 一個 harness 缺陷（第一輪就掉出來，已修）

第一次執行時 `l0-t3` 三格全部 `CARET_NOT_AT_ANCHOR`，**兩瀏覽器逐格相同**——
而那個相同正是歸因的依據。那三個錨點是**標題**，行框 520 twips；harness 瞄行中、
游標回報行頂，差 260 twips，被一個 195 twips 的常數擋掉。**195 是清單語料行距的
一半**，游標其實一直在對的行上，產品自己也接受了（它用游標矩形高度的一半＝260）。
修法不是把常數調大，而是讓**量到的幾何**說話：錨點矩形隨點擊一起傳進去，判準改成
「兩個行框重疊超過游標高度的一半」——沒有常數，而相鄰行完全不重疊，永遠不可能通過。
清單那一半維持舊規則（它用掃描找錨點，沒有錨點矩形可比，而且它的輪次已經錄好了）。
...
### 9.5.13 D5 第七輪：四格在**同一代殼層**上、用**強化後的判準**重跑（2026-08-16）

round 6 那次 PASS 是真的，但它綁在殼層 v6 上，而 051／052 之後殼層走到了 v8；
判定也是用 round one 的規則做的。第七輪把這兩件一起收掉：

| | |
|---|---|
| 四格 | **全部 PASS**（`--criteria round-two`） |
| 事件 | **411 個，全部 `isTrusted: true`，零合成** |
| 殼層 | **v8 `4daad6b4…`**，由 `session-attestation-2.json` 從頁面外面證言前後未變 |
| artifact | `572035ac…`（未動） |

強化後的判準實際咬到的東西：拖曳兩格的 `<text:list>` 由 2 → 3 → 4（**打開文件數的**，
不是「有沒有攔到檔」）；IME 那格 **兩次 commit、修訂號正好前進兩次**，而且
`中文輸入`／`阿卍` 兩串**都在存出來的文件裡**；剪貼簿那格 **copy 與 paste 兩個都要有**。

**round 6 的證據與判定原樣保留**——它是照當天的規則誠實判的，這一輪不改寫它。

這也是**第一輪跑在 051／052 都修好的 build 上的人工輪**。前六輪的 operator 是在
「點在行下半放不了游標、點在文字外也放不了、每次還要等 30 秒」的產品上操作的；

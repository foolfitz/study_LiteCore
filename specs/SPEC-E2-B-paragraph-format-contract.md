...
### 目前狀態：**9.7 的寫法暫停，等第二次裁決**

9.7 採用 B' 的決定沒有撤回，**但它指定的那個相等閘門已被實測打斷**。
已把兩個斷點與三個問題送回同一位裁決者（同一個 agent，續談）：
裝飾正規化之後檢查還會不會失敗、斷點二是不是結構性的、
以及若兩者都不可修，是否翻回 A——**若翻回 A，9.7 要改寫成明著的翻案，不是悄悄修訂**。

**在第二次裁決回來之前，不得把任何跨段的處置寫進 relink。**

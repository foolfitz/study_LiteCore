...
**身分閘門不是逐位元相等**，因為這些是段落層級的動作，要建立的身分是
「同一組**段落**」不是「同一段**字元範圍**」：

1. **行數相等**；
2. **block 數與派送前相等**（縮掉一段會同時掉一行與一個 block，兩條各自抓得到）；
3. 逐行文字相等，**允許最多剝掉一個裝飾前綴**，前綴取自**在這顆 build 上量過的封閉集合**；
4. **兩側都要正規化**——`set-list-none` 派在已經是清單的跨段選取上，
   裝飾會出現在**派送前**那一側；
5. 未知前綴 → typed cannot-verify，**永遠不算相等**。
...
第三輪採樣到 `.uno:DefaultNumbering` 讀回來是「    1. 」「    2. 」——**逐行遞增**。
裁決條件要的是「精確位元字串的封閉集合」；項目符號滿足（「    • 」九筆穩定），
**編號不滿足**，精確字串的集合隨清單長度無界。

顯然的修法是讓編號用**有界的樣式**而不是字面量，但那會以字面量不會的方式
遮住真的以「    12. 」開頭的文字，而那在散文裡是真實存在的形狀。
**已送回裁決者，未逕自採用。**

### 翻回 A 的條件（可檢查，由裁決者訂）

1. ~~跨段而第一段是部分的選取讀不回完整結構~~ ——**第三輪已測，不成立**；
2. 裝飾集合封不起來（編號／巢狀在輪與輪之間不穩）→ cannot-verify 變成常態 → **A**；

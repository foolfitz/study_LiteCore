...
> **「四路清單」那一列要讀窄一點（2026-08-15 補記，判定不變）**：
> 它證明的是四份清單**指同一組動作**，而它的「client」那一份是
> `editor-shell/editor-client.js` 與 `editor-shell-v2/paragraph-editor-client.js`
> **兩個殼層的聯集**。聯集本身沒有錯，但**其中一個殼層接不上這顆 profile**
> ——v1 的客戶端同時要求 capability `narrow-editor-v1` 與
> `editorContract.version === 1`，v2 兩個都不是。所以這一列**不能**讀成
> 「host 做得到這十五件事」；量到的是 v1 那十個在 v2 profile 上
> **沒有任何殼層到得了**（0／10，兩個殼層各自的理由不同，對照組是 v1 殼層在
> v1 profile 上 10／10）。**本判定的範圍是那五個段落動作，那五個確實跑過 132 個
> run，所以判定不受影響**；受影響的是把這一列外推成產品完整性的讀法。
> 處置寫在 [SPEC E2-C](./SPEC-E2-C-paragraph-format-validation.md) 2.2，
> 是該規格的**進場工作**（純殼層，零 relink，不動任何 artifact）。
...
| 日期 | 內容 |
|---|---|
| 2026-08-15 | **v13。補記一列的讀法，判定與門檻皆未動。** 9.10 的「四路清單各 15 個動作」被外推成「產品做得到十五件事」是錯的：那個檢查的 client 側是兩個殼層的**聯集**，而 v1 殼層接不上 v2 profile（capability 與 contract version 兩個閘都不符）。實測 0／10，對照組 v1 殼層在 v1 profile 上 10／10。**E2-B 的判定範圍是五個段落動作，不受影響**；缺口的處置是 [E2-C](./SPEC-E2-C-paragraph-format-validation.md) 的進場工作。 |

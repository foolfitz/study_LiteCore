...
> （**原始碼層級的推論**：那個呼叫不返回是量到的，擋法因此也不返回是推的一步，
> 尚未讓 barrier 自己走過那一格）。
...
### 11.8 收復**延後**，解除條件具名（2026-08-16，外部裁決）

§11.7 把兩條路都寫成合法：現在收復，或明確決定延後並寫下理由。**決定是延後。**
外部裁決（fable，使用者授權），理由不是「殼層感覺還在動」這種無法判定的說法：

**承重的理由是一個已經排定的決定點。** `e1-editor-v1` 自己帶著
[finding 045](../findings/045-inline-format-actions-discard-the-enabled-flag-and-toggle.md)
的缺陷，而它要不要 relink 已經排在
[`PLAN-E2-C-relink-v3.md`](../handoff/PLAN-E2-C-relink-v3.md) P3
（「等 v3 的參數化形式在產品上驗過再單獨決定」）。**若那個決定是 relink，
四個雜湊全部換新，現在做的殼層收復——含兩輪人工——百分之百作廢。**
在一個已排定、可能作廢全部四個雜湊的決定點之前對第四個雜湊單獨重取證，
是把 operator 時間押在一個已知的作廢風險上。

**歷史站在延後這邊，而且是量到的**：§11.4→§11.6 的時間線裡，08-06 那次
「立即收復」（含兩輪人工）**在 24 小時內被下兩次重建作廢**；中間那次重建
（E1-D）根本沒收復就被取代，而那是對的。收復輪自己還會撞上不相干的基礎設施
缺陷（§11.5 的 finding 023）。

**成本結構查證過：人工輪不隨改動量成長。** 每次收復都是固定的「四個自動相位
× 兩瀏覽器 ＋ 兩輪各九項人工」，實測一個晚上跑得完（08-07 的 23:41／23:48）。
**所以「愈晚愈貴」是錯的，方向還是反的**——每多收復一次就多付一整份固定成本。

**解除條件（任一成立即收復，不再另行裁示）**：

1. `e1-editor-v1` 的 relink 決定作出之後的**第一個 operator 時段**。
   若決定是 relink，做的是對新 artifact 的完整重取證（四個雜湊一起刷新）；
   若決定是不 relink，做的是對 `835b453d…` ＋當時殼層的收復。
   時程允許就與 E2-C D5 的四格人工排**同一個時段**（同一套設置：Fcitx5
   Chewing、真剪貼簿、真實指標事件）。
2. **引信**：任何具名事件要消費 `E1_GO_ODT_EDITOR` 的殼層那一半
   （上游送出重啟、demo 重新升級、外部引用），**即刻收復，不等第 1 條**。

**延後不是 finding 027 的形狀**，這一點機制上驗證過：027 是**錯置**——證據宣稱
描述一份它從沒跑過的東西；延後製造的是**過期**——證據誠實地描述一份不再出貨的
東西。凍結的 manifest 沒有被改寫（仍是 `56f2be28…`／`f9b1a52f…`），divergence 檔
記了路徑、兩個雜湊、日期、理由與代價，守衛強制維持這個區分。實測
`check_e1_c_bundle_intact.py`：exit 0、`intact: false`、`divergedAsDeclared` 三筆、
`problems: []`。

**否決的第三條路兩條**：

- **把殼層 bundle 拆成 E1 專用＋共用**：否。這在 §11.7 與 divergence 檔的
  `whyHere` 已經打過一次官司——只覆寫 `editor-shell-v2/` 會保住綁定，代價是
  E1 產品頁繼續帶著同一個缺陷，而且一棵樹兩種游標手勢。**綁定服務產品，
  不是反過來。**
- **先跑自動相位當半份證據**：否。產出一個沒有消費者、下次共用 session 一動
  就蒸發的 PARTIAL 形狀，而 PARTIAL 誇報正是 §11.4 補閘門要防的東西。
  要便宜的機器保證，正確尺寸的工具已經在跑：`check_e1_c_bundle_intact.py`
...
| 2026-08-16 | 補記一件 08-13 那條沒記到的（**文件性，未動閘門、未動收窄範圍、判定不變**）：`l4-stress-100` 那 100 個 body-level as-char frame **連存檔都活不過**。E2-C 的 D3 語料輪量到系統 LibreOffice 26.8 與 `build-native-26-8` 在**完全沒有編輯**的 `--convert-to odt` 下都把 100 個全部丟掉（frames 100 → 0，ZIP 裡的 `Pictures/` 一併消失）。因此**任何斷言那 100 個會保存下來的檢查，斷言的是沒有一個 LibreOffice 做得到的事**；E2-C 的結構判準已改成與「什麼都不做的存檔」比較。證據：`findings/evidence/sdk-e2/e2-c-validation/d3-corpus/`。 |
